...
class User:
    ...
    def builditemtable(self):
         ...
         scrollbarx = ttkbootstrap.Scrollbar(self.tableframe, orient=HORIZONTAL,bootstyle='secondary')
         scrollbary = ttkbootstrap.Scrollbar(self.tableframe, orient=VERTICAL,bootstyle='secondary')
         self.columns = ("Product ID", "Product Name", "Description", "Category",
                    'Price', 'Stocks',)
         self.tree = ttkbootstrap.Treeview(self.tableframe, bootstyle='info',columns=self.columns, selectmode="extended", height=18,
    ...
    def make_invoice(self):
        ...
        scrollbarx = ttkbootstrap.Scrollbar(self.tableframe1, orient=HORIZONTAL,bootstyle='secondary')
        scrollbary = ttkbootstrap.Scrollbar(self.tableframe1, orient=VERTICAL,bootstyle='secondary')
        self.tree = ttkbootstrap.Treeview(self.tableframe1,bootstyle='info', columns=("Transaction ID", "Product ID", "Product Name",
        'Quantity', 'Price','Date','Time','user','priority'), selectmode="browse", height=6,yscrollcommand=scrollbary.set, xscrollcommand=scrollbarx.set)
        self.tree.column('#0', stretch=NO, minwidth=0, width=0)
        self.tree.column('#1', stretch=NO, minwidth=0, width=130)
    ...
    def user_input(self):
       ...
       Button(self.entryframe, text="Add ", command=self.addtotrans, bd=10, width=10, height=3,bg="#FFFFFF",font="roboto 10").place(x=100,y=80)
       ...
       self.invoice = int(0 if  self.invoice[0][0] is None else self.invoice[0][0])+ 1
    ...
    def addtotrans(self):
        if(len(self.additem.get()) == 0 or self.inventory.count(self.additem.get())==0):
            ...
            if(int(self.qty.get()) <= 0):
                messagebox.showerror('Error', 'Invalid quantity!')
                return
            self.cur.execute("select productid,product_desc from products where product_desc = ? ",(self.additem.get(),))
            row = self.cur.fetchall()
            row = [list(row[0])]
            row[0].insert(0,self.transid)
            self.transid+=1
            row[0].append(int(self.qty.get()))
            row[0].append((int(self.qty.get())*int(self.desc_price[self.additem.get()])))
            x=str(datetime.datetime.now().strftime("%d-%m-%y"))
            row[0].append(x)

            x=datetime.datetime.now()
            x=str(x.hour)+' : '+str(x.minute)+' : '+str(x.second)
            capuser = Addtional_features.user
            capuser = capuser.upper()
            self.cur.execute("select account_type from users where username= ? ", (capuser,))
            l = self.cur.fetchall()
            self.accountt_type = l[0][0]
            if  self.accountt_type == 'ADMIN':
                self.priority='high'
            else:
                self.priority='low'
            ...
            row[0].append(x)
            row[0].append(Addtional_features.user)
            row[0].append(self.priority)
            row = [tuple(row[0])]
            self.cartitemid.set(row[0][1])
            self.cartitemprice.set(self.desc_price[self.additem.get()])
            self.cartitem.set(row[0][2])
            self.cur.execute("select stocks from products where productid=?", (row[0][1],))
            li = self.cur.fetchall()
            ...
            self.total.set(int(self.total.get()) + (int(self.qty.get()) * int(self.desc_price[self.additem.get()])))
            self.qty.set('1')
            self.additem.set('')
    ...
    def transtableadd(self):
        ...
        for i in a:
            s = (str(i[5])).split('-')
            i[5] = s[2] + "-" + s[1] + "-" + s[0]
            self.cur.execute("insert into sales values (?,?,?,?,?,?,?,? )",(int(i[0]),int(self.invoice),int(i[1]),int(i[3]),i[5],i[6],i[7],i[8]))
            self.cur.execute("select stocks from products where productid=?",(int(i[1]),))
            l=self.cur.fetchall()
            self.cur.execute("update products set stocks=? where productid=?",(l[0][0],int(i[1])))
            self.base.commit()
